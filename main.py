from config import API_ID, API_HSH, admin
...
# فایل config.py ادیت شود
# coded by https://t.me/Amirhaminjast

@amir.on(events.NewMessage(pattern="/start"))
async def start_message(event: Message):
    button_1 = Button.inline(text="شروع بمبر", data="bomber")
    await event.reply("**سلام به ربات اس ام اس بمبر خوش آمدید**", buttons=[button_1])

@amir.on(events.CallbackQuery(data="bomber"))
async def callback_bobmber(event: Message):
    async with amir.conversation(event.sender_id) as conv:
        await conv.send_message("**شماره کاربر را همراه با صفر ارسال کنید**")
        response = await conv.get_response()
        response2 = (response.text)
        print(response2)
        both(response2)
        await conv.send_message("**کاربر در لیست بمبر لیست اضافه شد**")
...
def bomb(number):
    ...
    # An SMS is sent every 1 seconds
